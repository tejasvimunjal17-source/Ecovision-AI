...
def update_status(complaint_id, new_status, changed_by, note=""):
    execute("UPDATE complaints SET status=?, updated_at=CURRENT_TIMESTAMP WHERE id=?",
            (new_status, complaint_id))
    if new_status == "Resolved":
        execute("UPDATE complaints SET resolved_at=CURRENT_TIMESTAMP WHERE id=?", (complaint_id,))
