...
st.set_page_config(page_title="Admin Panel | EcoVision AI", page_icon="🛠️", layout="wide")
require_login(allowed_roles=["admin"])
load_css()

st.markdown('<div class="eco-hero"><h1>🛡️ EcoVision AI Admin Panel</h1><p>Separate administrative control center for users, complaints, categories, officers and system-wide analytics.</p></div>', unsafe_allow_html=True)
...
with tab_users:
    ...
    if citizens:
        ...
        if st.button("Toggle Active/Inactive"):
            ...
            if u:
                execute("UPDATE users SET is_active=? WHERE id=?", (not bool(u["is_active"]), uid))
...
with tab_settings:
    ...
    st.write("**Database:** Supabase PostgreSQL · " + (cfg.SUPABASE_URL or "Not configured"))
