...
# ---- Supabase (primary persistent data store) --------------------------
# All application records are stored in Supabase PostgreSQL. The service-role
# key is used server-side only; it is never rendered into the browser.
...
SUPABASE_CONFIG = _SupabaseConfig()

# Kept only for backwards compatibility with old code/imports. The upgraded
# app does not write application data to a local SQLite database.
